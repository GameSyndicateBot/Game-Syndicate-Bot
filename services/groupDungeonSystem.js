...
const WINDOWS = [{ startH: 11, startM: 0, lastH: 11, lastM: 19, closeH: 12, closeM: 9, key: 'day' }, { startH: 18, startM: 0, lastH: 18, lastM: 19, closeH: 19, closeM: 9, key: 'evening' }];
...
async function start(interaction,id){const g=getGroup(id);if(!g||g.status!=='forming'||g.leader_id!==interaction.user.id)return ephemeral(interaction,{content:'Начать рейд может только лидер.'});if(!canStartNow())return ephemeral(interaction,{content:'Время старта истекло. Новый рейд можно запустить только до 11:19 или 18:19 МСК.'});const ms=members(id);if(ms.length<MIN_PLAYERS)return ephemeral(interaction,{content:`Нужно минимум ${MIN_PLAYERS} участника. Сейчас: ${ms.length}.`});const a=analyze(id),now=Date.now(),end=now+RAID_DURATION_MS;db.prepare("UPDATE dungeon_groups SET status='active',success_chance=?,started_at=?,ends_at=? WHERE id=?").run(a.chance,new Date(now).toISOString(),new Date(end).toISOString(),id);for(const m of ms){db.prepare("UPDATE heroes SET status='dungeon',updated_at=CURRENT_TIMESTAMP WHERE user_id=? AND status='ready'").run(m.user_id);}await interaction.update({embeds:[groupEmbed(id,false)],components:[]});await ensureHub(interaction.client,interaction.guildId);}
...
async function tick(client){const due=db.prepare("SELECT * FROM dungeon_groups WHERE status='active' AND ends_at<=?").all(new Date().toISOString());for(const g of due)await resolveGroup(client,g).catch(e=>console.error('[Dungeon] resolve',g.id,e));await ensureHub(client).catch(()=>{});}
