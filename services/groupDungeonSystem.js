...
function hubRows(){const open=Boolean(currentWindow());return [new ActionRowBuilder().addComponents(
 ...
 new ButtonBuilder().setCustomId('dng_queue_list').setLabel('Кто записался').setEmoji('👥').setStyle(ButtonStyle.Secondary).setDisabled(!open),
...
async function queueList(interaction){
 const ctx=windowContext();if(!ctx)return ephemeral(interaction,{content:'Запись сейчас закрыта.'});
 const rows=db.prepare('SELECT * FROM dungeon_queue_entries WHERE guild_id=? AND window_key=? ORDER BY joined_at').all(interaction.guildId,ctx.key);
 if(!rows.length)return ephemeral(interaction,{content:'В текущее окно пока никто не записался.'});
 const counts={tank:0,healer:0,dps:0,support:0};for(const r of rows)counts[r.role_pref]=(counts[r.role_pref]||0)+1;
 const lines=rows.slice(0,40).map((r,i)=>`${i+1}. <@${r.user_id}> — **${CLASS_LABELS[normalizeClassKey(r.class_key)]||r.class_key}** · ${ROLE_LABELS[r.role_pref]||r.role_pref}`);
 const summary=`🛡️ ${counts.tank||0} · 💚 ${counts.healer||0} · ⚔️ ${counts.dps||0} · 🌀 ${counts.support||0}`;
 return ephemeral(interaction,{embeds:[new EmbedBuilder().setColor(0x7c3aed).setTitle(`👥 Записались в подземелье — ${rows.length}`).setDescription(`${summary}\n\n${lines.join('\n')}`.slice(0,4000)).setFooter({text:'Показывается класс и роль, с которыми игрок записался.'})]});
}
