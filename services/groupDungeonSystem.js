...
async function start(interaction,id){const g=getGroup(id);if(!g||g.status!=='forming'||g.leader_id!==interaction.user.id)return ephemeral(interaction,{content:'Начать рейд может только лидер.'});if(!canStartNow())return ephemeral(interaction,{content:'Время старта истекло. Новый рейд можно запустить только до 11:19 или 18:19 МСК.'});const ms=members(id);if(ms.length<MIN_PLAYERS)return ephemeral(interaction,{content:`Нужно минимум ${MIN_PLAYERS} участника. Сейчас: ${ms.length}.`});const busy=ms.filter(m=>{const h=getHero(m.user_id);const expedition=db.prepare("SELECT 1 FROM hero_expeditions WHERE user_id=? AND status='active' LIMIT 1").get(m.user_id);return !h||h.status!=='ready'||Boolean(expedition);});if(busy.length)return ephemeral(interaction,{content:`❌ Рейд не запущен. Эти герои заняты или находятся в экспедиции:
