...
function migrateLegacyResources() {
  ...
  // V20.2.2: объединение дубликата Пряных трав после миграций.
  ...
  if (duplicateSpicyHerbs.length) {
    const mergeSpicy = db.transaction(() => {
      ...
      for (const row of duplicateSpicyHerbs) upsert.run(row.user_id, Number(row.quantity)||0);
