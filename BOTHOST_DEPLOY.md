...
## Регистрация Discord-команд для DEV и PROD

Добавь в переменные окружения:

```env
PROD_GUILD_ID=ID_ОСНОВНОГО_СЕРВЕРА
DEV_GUILD_ID=ID_DEV_СЕРВЕРА
```

Команды деплоя:

```bash
npm run deploy:dev     # только DEV-сервер, команды появляются почти сразу
npm run deploy:prod    # только основной сервер
npm run deploy:all     # оба сервера
npm run deploy:global  # глобальная регистрация для всех серверов
```

Старая переменная `GUILD_ID` продолжает работать как основной сервер. Также можно зарегистрировать команды на произвольном сервере:

```bash
node deploy-commands.js ID_СЕРВЕРА
```
