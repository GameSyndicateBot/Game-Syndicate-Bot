...
function drawPack(ctx, x, y, w, h, pack, balance, accent) {
    ...
    ctx.fillText(pack.id === 'base' ? 'B' : pack.id === 'premium' ? 'P' : 'E', x + 72, y + 88);
    ctx.textAlign = 'left';
    ...
    rows.forEach((row, i) => ctx.fillText(row, x + 38 + (i % 2) * 205, y + 160 + Math.floor(i / 2) * 34));
    const enough = balance >= pack.cost;
    ctx.fillStyle = enough ? colors.green : colors.red; ctx.font = 'bold 19px Arial';
    ctx.fillText(enough ? 'ДОСТУПЕН' : `НЕ ХВАТАЕТ ${pack.cost - balance}`, x + 38, y + h - 38);
    ctx.restore();
}
...
async function createCardShopPanel(user, data = {}) {
    ...
    const accents = [colors.green, colors.purpleLight, colors.gold];
    packs.forEach((pack, i) => drawPack(ctx, 85 + i * 505, 315, 465, 410, pack, balance, accents[i]));
    const min = Math.min(...packs.map(p => p.cost));
    drawProgressBar(ctx, 90, 790, 1420, 22, Math.min(balance, min), min, balance >= min ? colors.green : colors.red);
    ctx.fillStyle = balance >= min ? colors.green : colors.red; ctx.font = 'bold 21px Arial'; ctx.textAlign = 'center';
    ctx.fillText(balance >= min ? 'Можно купить Base Pack' : `До Base Pack не хватает ${min - balance} Dust`, WIDTH / 2, 835);
