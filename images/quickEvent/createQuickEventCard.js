...
async function createQuickEventCard(event, phase='active'){
  ...
  if(event.media){
    ...
    const displayPrompt=event.type==='emoji_riddle'
      ?'ЭМОДЗИ ПОКАЗАНЫ В СООБЩЕНИИ DISCORD'
      :(event.prompt||event.display||'');
    const lines=wrap(ctx,displayPrompt,1050);let yy=330-(lines.length-1)*30;for(const line of lines){ctx.fillText(line,700,yy);yy+=68;}
