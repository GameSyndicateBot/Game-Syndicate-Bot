...
async function createHeroCard(hero,user){
 const c=createCanvas(1500,900),ctx=c.getContext('2d'); const cls=HERO_CLASSES[hero.display_class_key||hero.class_key]||HERO_CLASSES[hero.class_key],org=ORIGINS[hero.origin_key];
 ...
 ctx.fillStyle='#C084FC';ctx.font='bold 32px Arial';ctx.fillText(`${cls.name}  •  ${org.name}`,390,210);
 ...
 const sx=85,sy=500,w=300,gap=32;stat(ctx,sx,sy,w,'HP',`${hero.hp}/${hero.max_hp}`);stat(ctx,sx+w+gap,sy,w,'СИЛА',hero.strength);stat(ctx,sx+(w+gap)*2,sy,w,'ЛОВКОСТЬ',hero.dexterity);stat(ctx,sx+(w+gap)*3,sy,w,'ИНТЕЛЛЕКТ',hero.intelligence);const bx=251;stat(ctx,bx,sy+115,w,'МУДРОСТЬ',hero.wisdom||0);stat(ctx,bx+w+gap,sy+115,w,'ВЫНОСЛИВОСТЬ',hero.vitality||0);stat(ctx,bx+(w+gap)*2,sy+115,w,'УДАЧА',hero.luck);
