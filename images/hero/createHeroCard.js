...
async function createHeroCard(hero,user){
 ...
 const sx=85,sy=505,w=410,gap=35;stat(ctx,sx,sy,w,'HP',`${hero.hp}/${hero.max_hp}`);stat(ctx,sx+w+gap,sy,w,'СИЛА',hero.strength);stat(ctx,sx+(w+gap)*2,sy,w,'ЗАЩИТА',hero.defense);stat(ctx,sx,sy+115,w,'ЛОВКОСТЬ',hero.dexterity);stat(ctx,sx+w+gap,sy+115,w,'ИНТЕЛЛЕКТ',hero.intelligence);stat(ctx,sx+(w+gap)*2,sy+115,w,'УДАЧА',hero.luck);
