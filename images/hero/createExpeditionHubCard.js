...
function locationPanel(ctx, x, y, w, h, location, accent) {
  ...
  const descSize = fitText(ctx, location.description, w - 56, 18, 14);
  ctx.font = `${descSize}px Arial`;
  ctx.fillStyle = '#a78bfa';
  ctx.fillText(location.description, x + 28, y + 151);
...
async function createExpeditionHubCard({ world, nextBossLabel, locked }) {
  ...
  ctx.textAlign = 'right';
  ctx.fillText('GAME SYNDICATE • EXPEDITION HUB V16.1.1', 1392, 858);
  ctx.textAlign = 'left';
