...
function scheduleNextQuickEvent(delay = randomDelay()) {
  ...
  nextEventAt = Date.now() + safeDelay;
  ...
  schedulerTimer = setTimeout(async () => {
    ...
    schedulerPosting = true;

    try {
      await postQuickEvent(schedulerClient);
    } catch (error) {
      console.error('[QuickEvent]', error);
    } finally {
      schedulerPosting = false;
      scheduleNextQuickEvent();
    }
  }, safeDelay);
...
function getQuickEventScheduleStatus() {
  return {
    nextEventAt,
    remainingMs: nextEventAt ? Math.max(0, nextEventAt - Date.now()) : null,
    posting: schedulerPosting,
  };
}

async function forceCloseQuickEvent(client, closedById) {
  initTables();
  ...
  if (round) {
    ...
    if (channel?.isTextBased()) {
      const sourceMessage = round.message_id
        ? await channel.messages.fetch(round.message_id).catch(() => null)
        : null;

      const closeText = `## 🛑 QUICK EVENT закрыт вручную\nСобытие завершено владельцем <@${closedById}> без выдачи награды.`;

      if (sourceMessage) {
        await sourceMessage.reply({
          content: closeText,
          allowedMentions: { users: [] },
        }).catch(() => channel.send({ content: closeText }));
      } else {
        await channel.send({ content: closeText });
      }
    }
  }

  const scheduledAt = scheduleNextQuickEvent();

  return {
    closed: Boolean(round),
    roundId: round?.id ?? null,
    scheduledAt,
    delayMs: scheduledAt ? scheduledAt - Date.now() : null,
  };
}
...
function startQuickEventScheduler(client) {
  ...
  if (!last || age >= MAX_INTERVAL_MS) {
    scheduleNextQuickEvent(2500);
  } else {
    const min = Math.max(0, MIN_INTERVAL_MS - age);
    const max = Math.max(min, MAX_INTERVAL_MS - age);
    scheduleNextQuickEvent(min + Math.floor(Math.random() * (max - min + 1)));
  }

  console.log('[QuickEvent] Запущено: события случайно через 1–1.5 часа');
}
