...
function getWorld(guildId = 'global') {
  ...
  return rows.map(row => {
    ...
    return { ...row, ...data, stage, event, nextThreshold:nextThreshold(stage) };
  });
}

function getRegion(guildId, regionKey) {
  return getWorld(guildId).find(r => r.region_key === regionKey || r.key === regionKey) || null;
}

function getContracts(guildId = 'global', regionKey = null) {
  ensureWorld(guildId);
  const sql = regionKey
    ? 'SELECT * FROM world_contracts WHERE guild_id=? AND date_key=? AND region_key=? ORDER BY id'
    : 'SELECT * FROM world_contracts WHERE guild_id=? AND date_key=? ORDER BY region_key,id';
...
function progressContract(guildId, regionKey, type, amount) {
  ...
  db.prepare('UPDATE world_contracts SET progress=?, completed=?, completed_at=CASE WHEN ?=1 THEN CURRENT_TIMESTAMP ELSE completed_at END WHERE id=?')
    .run(progress, progress >= row.target ? 1 : 0, progress >= row.target ? 1 : 0, row.id);
}
...
function applyExpeditionResult(guildId = 'global', location, result = {}) {
  ...
  db.prepare(`UPDATE world_regions SET reputation=reputation+?, expeditions=expeditions+1,
    successes=successes+?, materials_collected=materials_collected+?, last_activity_at=CURRENT_TIMESTAMP
    WHERE guild_id=? AND region_key=?`).run(gain, success ? 1 : 0, (result.materials || []).reduce((a,m)=>a+Number(m.quantity||0),0), guildId, regionKey);
  progressContract(guildId, regionKey, 'expeditions', 1);
  progressContract(guildId, regionKey, 'successes', success ? 1 : 0);
  progressContract(guildId, regionKey, 'materials', (result.materials || []).reduce((a,m)=>a+Number(m.quantity||0),0));
  progressContract(guildId, regionKey, 'reputation', gain);
  const state = maybeUnlockRegions(guildId);
  return { regionKey, reputationGain:gain, worldStage:state.worldStage };
}
...
function getWorldProgress(guildId='global') {
  ...
  const total = rows.reduce((a,r)=>a+Number(r.reputation||0),0);
...
module.exports = { dateKey, ensureWorld, getWorld, getRegion, getContracts, applyExpeditionResult, getWorldProgress, stageForReputation };
