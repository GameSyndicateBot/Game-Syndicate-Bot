...
function setGameLobbyRuntime(api,client){runtime={api,client};if(timer)clearInterval(timer);closeExpiredGameLobbies().catch(console.error);timer=setInterval(()=>closeExpiredGameLobbies().catch(console.error),60000);timer.unref?.();console.log('✅ GS Game Lobby: автозакрытие через 4 часа включено');}
async function publishGameLobby({creatorId,creatorName,game,mapName,lobbyCode}){
 ...
 if(runtime.api&&tg){const tm=await runtime.api('sendMessage',{chat_id:tg,...(thread?{message_thread_id:Number(thread)}:{}),text:tgText(l),parse_mode:'HTML',disable_web_page_preview:true,disable_notification:false});db.prepare('UPDATE game_lobbies SET telegram_chat_id=?,telegram_thread_id=?,telegram_message_id=? WHERE id=?').run(String(tg),thread?Number(thread):null,tm.message_id,l.id);await runtime.api('pinChatMessage',{chat_id:tg,message_id:tm.message_id,disable_notification:false}).catch(()=>null);const u=setTimeout(()=>runtime.api('unpinChatMessage',{chat_id:tg,message_id:tm.message_id}).catch(()=>null),60000);u.unref?.();}
