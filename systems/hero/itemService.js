...
function getEquipmentBonuses(userId){const total=Object.fromEntries(STAT_KEYS.map(k=>[k,0]));for(const item of getEquipment(userId)){const b=parseBonuses(item.bonuses_json);for(const k of STAT_KEYS)total[k]+=Number(b[k]||0);}return total;}
