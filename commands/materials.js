...
module.exports = {
  ...
  async execute(interaction) {
    ...
    return interaction.reply({ embeds:[new EmbedBuilder().setColor(0x7C3AED).setTitle(`🧱 Материалы: ${hero.name}`).setDescription(text).setFooter({ text:'Материалы пригодятся в следующем обновлении для крафта.' })], flags:MessageFlags.Ephemeral });
  },
};
