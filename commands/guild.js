...
async function showInventory(interaction, notice = '') {
  ...
  const slots=['melee','ranged','offhand','ring1','ring2','belt','legs','chest','boots','helmet','amulet'];
...
function createCompanionListing(userId,id,price){const value=Math.max(1,Number(price)||0);if(value<1)return {ok:false,reason:'price'};let data;try{db.transaction(()=>{data=takeCompanionForTransfer(userId,Number(id));if(!data)throw new Error('missing');db.prepare('INSERT INTO market_companion_listings(seller_id,companion_key,companion_name,companion_kind,rarity,price,companion_data) VALUES(?,?,?,?,?,?,?)').run(String(userId),data.key,data.name,data.kind,data.rarity,value,JSON.stringify(data));})();return {ok:true,data,price:value};}catch(e){return {ok:false,reason:e.message};}}
function buyCompanionListing(userId,id){const uid=String(userId);let lot;try{db.transaction(()=>{lot=db.prepare("SELECT * FROM market_companion_listings WHERE id=? AND status='open'").get(Number(id));if(!lot||lot.seller_id===uid)throw new Error('missing');const pay=removeCardDust(uid,Number(lot.price));if(!pay?.ok)throw new Error('dust');addCardDust(lot.seller_id,Number(lot.price));giveTransferredCompanion(uid,JSON.parse(lot.companion_data));const u=db.prepare("UPDATE market_companion_listings SET status='sold',buyer_id=?,closed_at=CURRENT_TIMESTAMP WHERE id=? AND status='open'").run(uid,Number(id));if(!u.changes)throw new Error('missing');})();return {ok:true,lot};}catch(e){return {ok:false,reason:e.message};}}
function cancelCompanionListing(userId,id){let lot;try{db.transaction(()=>{lot=db.prepare("SELECT * FROM market_companion_listings WHERE id=? AND seller_id=? AND status='open'").get(Number(id),String(userId));if(!lot)throw new Error('missing');giveTransferredCompanion(userId,JSON.parse(lot.companion_data));db.prepare("UPDATE market_companion_listings SET status='cancelled',closed_at=CURRENT_TIMESTAMP WHERE id=?").run(Number(id));})();return {ok:true,lot};}catch(e){return {ok:false,reason:e.message};}}

const marketDrafts=new Map();
function removeEquipmentForTrade(userId,inventoryId){const row=db.prepare('SELECT * FROM hero_inventory WHERE id=? AND user_id=?').get(inventoryId,String(userId));if(!row||!ITEMS[row.item_key]?.slot||Number(row.quantity)<=0)return null;if(Number(row.quantity)<=1){db.prepare('DELETE FROM hero_equipment WHERE user_id=? AND inventory_id=?').run(String(userId),inventoryId);db.prepare('DELETE FROM hero_class_equipment WHERE user_id=? AND inventory_id=?').run(String(userId),inventoryId);db.prepare('DELETE FROM hero_inventory WHERE id=?').run(inventoryId);}else db.prepare('UPDATE hero_inventory SET quantity=quantity-1 WHERE id=?').run(inventoryId);return {key:row.item_key,name:ITEMS[row.item_key]?.name||row.item_key,upgrade:Number(row.upgrade_level||0)};}
...
async function handleModal(interaction) {
  ...
  if(parts[0]==='guild'&&parts[1]==='market'){
    ...
    if(kind==='exchangesearch'){
      ...
      const matches=Object.entries(ITEMS).filter(([,x])=>x.slot&&(x.name||'').toLocaleLowerCase('ru-RU').includes(query)).slice(0,25);
