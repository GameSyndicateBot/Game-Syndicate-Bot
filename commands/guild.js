...
const { getEffectiveHero, getInventory, getEquipment, getInventoryItem, grantItem, equipItem, unequipItem, unequipInventoryItem, formatBonuses } = require('../systems/hero/itemService');
...
const { getActiveExpedition, getTreasureMapState, activateTreasureMap } = require('../systems/hero/expeditionService');
...
const { listOpen: listEquipmentMarket, listMine: listMyEquipmentMarket, duplicateEquipment, sellableConsumables: marketConsumables, createListing, buyListing, cancelListing, sellToBlacksmith } = require('../systems/hero/equipmentMarketService');
const { formatItemDescription } = require('../systems/hero/itemPresentation');
...
async function showInventory(interaction, notice = '') {
  ...
  const allInventory=getInventory(interaction.user.id,{limit:100}),items=allInventory.filter(i=>i.item_type!=='utility'),utilities=allInventory.filter(i=>i.item_type==='utility'),equipment=getEquipment(interaction.user.id),equippedIds=new Set(equipment.map(x=>Number(x.inventory_id)));
  ...
  let artifactRows=[];try{artifactRows=db.prepare(`SELECT hae.slot_no,hi.id inventory_id,i.name,i.rarity FROM hero_artifact_equipment hae JOIN hero_inventory hi ON hi.id=hae.inventory_id JOIN hero_items i ON i.item_key=hi.item_key WHERE hae.user_id=? ORDER BY hae.slot_no`).all(String(interaction.user.id));}catch(_){}
  const artifactMap=new Map(artifactRows.map(x=>[Number(x.slot_no),x]));
  const artifactLines=[1,2].map(slot=>{const x=artifactMap.get(slot);return `💠 **Артефакт ${slot}:** ${x?`${raritySphere(x.rarity)} ${x.name} (#${x.inventory_id})`:'⚪ Свободно'}`;}).join('\n');
  ...
  const utilityText=utilities.length?utilities.map(i=>`${raritySphere(i.rarity)} **#${i.id} ${i.name}** ×${i.quantity}`).join('\n'):'Расходных походных предметов нет.';
  ...
  const embed=new EmbedBuilder().setColor(0x8B5CF6).setTitle(`🎒 Инвентарь — ${hero.name}`).setDescription([notice,'### Активные слоты',slotText,mountLine,artifactLines,legacy,'','### Предметы',itemText,'','### 🗺️ Походные предметы',utilityText,'','### 🐎 Маунты',mountStock,'','### 🐾 Питомцы',petStock].filter(Boolean).join('\n').slice(0,4000)).setFooter({text:'⚪ Common • 🔵 Rare • 🟣 Epic • 🟠 Legendary • 🔴 Mythic. Активные предметы отмечены текстом «надето».'});
  ...
  if(utilities.some(i=>i.item_key==='treasure_map') || getTreasureMapState(interaction.user.id)) inventoryActions.push(
    new ButtonBuilder().setCustomId('guild:inventory:treasuremap').setLabel(getTreasureMapState(interaction.user.id)?'Карта уже активна':'Использовать карту сокровищ').setEmoji('🗺️').setStyle(ButtonStyle.Primary).setDisabled(getTreasureMapState(interaction.user.id))
  );
  inventoryActions.push(new ButtonBuilder().setCustomId('guild:artifacts').setLabel('Артефакты').setEmoji('💍').setStyle(ButtonStyle.Secondary));
  components.push(new ActionRowBuilder().addComponents(inventoryActions));
...
async function showBlacksmith(interaction, notice = '', category = 'all') {
  ...
  // Надетая строка инвентаря может содержать несколько одинаковых экземпляров.
  // В таком случае один экземпляр остаётся надетым, а свободные копии доступны для переработки.
  const dismantleItems = getInventory(interaction.user.id,{limit:100}).filter(i => i.slot && (!equippedIds.has(Number(i.id)) || Number(i.quantity||0) >= 2));
  if (dismantleItems.length) components.push(new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder().setCustomId('guild:blacksmith:dismantle').setPlaceholder('♻️ Разобрать свободную экипировку')
      .addOptions(dismantleItems.slice(0,25).map(i => ({
        ...
        description:`${RARITY_LABELS[i.rarity] || i.rarity} · доступно копий: ${equippedIds.has(Number(i.id))?Math.max(0,Number(i.quantity||0)-1):Number(i.quantity||0)} · будет уничтожена 1`.slice(0,100)
      })))
  ));
  components.push(new ActionRowBuilder().addComponents(
...
async function showArtifacts(interaction, notice='') {
  const hero=getHero(interaction.user.id);
  if(!hero)return interaction.reply({content:'❌ Сначала создай героя.',flags:MessageFlags.Ephemeral});
  let equipped=[];
  try{equipped=db.prepare(`SELECT hae.slot_no,hi.id inventory_id,i.name,i.rarity,i.description,i.bonuses_json
    ...
    WHERE hae.user_id=? ORDER BY hae.slot_no`).all(String(interaction.user.id));}catch(_){}
  const equippedIds=new Set(equipped.map(x=>Number(x.inventory_id)));
  const items=getInventory(interaction.user.id,{type:'artifact',limit:50});
  const free=items.filter(i=>!equippedIds.has(Number(i.id)));
  const slotLines=[1,2].map(slot=>{
    const row=equipped.find(x=>Number(x.slot_no)===slot);
    if(!row)return `💠 **Слот ${slot}:** ⚪ свободен`;
    const bonuses=formatBonuses(row.bonuses_json);
    return `💠 **Слот ${slot}:** ${raritySphere(row.rarity)} **${row.name}** (#${row.inventory_id})${bonuses.length?`\n${bonuses.join(' · ')}`:''}`;
  }).join('\n\n');
  const stock=free.length?free.map(i=>{
    const bonuses=formatBonuses(i.bonuses_json);
    return `${raritySphere(i.rarity)} **#${i.id} ${i.name}** · ${RARITY_LABELS[i.rarity]||i.rarity}\n${i.description||''}${bonuses.length?`\n${bonuses.join(' · ')}`:''}`;
  }).join('\n\n'):'Свободных артефактов нет.';
  const embed=new EmbedBuilder().setColor(0xA855F7).setTitle('💍 Артефакты героя')
    .setDescription([notice,'### Экипированные артефакты',slotLines,'','### Хранилище',stock].filter(Boolean).join('\n').slice(0,4000))
    .setFooter({text:'Можно одновременно экипировать два артефакта. Их бонусы действуют только пока артефакт находится в слоте.'});
  const components=[];
  if(free.length)components.push(new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder().setCustomId('guild:artifacts:equipselect').setPlaceholder('Выбрать артефакт для экипировки')
      .addOptions(free.slice(0,25).map(i=>({label:`#${i.id} ${i.name}`.slice(0,100),value:String(i.id),emoji:'💍',description:`${RARITY_LABELS[i.rarity]||i.rarity} · выбрать слот`})))
  ));
  if(equipped.length)components.push(new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder().setCustomId('guild:artifacts:remove').setPlaceholder('Снять артефакт')
      .addOptions(equipped.map(i=>({label:`Слот ${i.slot_no}: ${i.name}`.slice(0,100),value:String(i.slot_no),emoji:'📤'})))
  ));
  components.push(new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('guild:inventory').setLabel('Назад в инвентарь').setEmoji('⬅️').setStyle(ButtonStyle.Secondary)
  ));
  const payload={embeds:[embed],components};
  const eph=Boolean(interaction.message?.flags?.has?.(MessageFlags.Ephemeral));
  return eph?interaction.update(payload):interaction.reply({...payload,flags:MessageFlags.Ephemeral});
}

async function showArtifactSlotChoice(interaction, inventoryId) {
  const item=getInventoryItem(interaction.user.id,Number(inventoryId));
  if(!item || item.item_type!=='artifact')return showArtifacts(interaction,'❌ Артефакт не найден.');
  return interaction.update({
    embeds:[new EmbedBuilder().setColor(0xA855F7).setTitle(`💍 ${item.name}`).setDescription('Выбери один из двух слотов. Если слот занят, прежний артефакт будет снят.')],
    components:[new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId(`guild:artifacts:equip:${inventoryId}:1`).setLabel('Надеть в слот 1').setEmoji('1️⃣').setStyle(ButtonStyle.Success),
      new ButtonBuilder().setCustomId(`guild:artifacts:equip:${inventoryId}:2`).setLabel('Надеть в слот 2').setEmoji('2️⃣').setStyle(ButtonStyle.Success),
      new ButtonBuilder().setCustomId('guild:artifacts').setLabel('Отмена').setStyle(ButtonStyle.Secondary)
    )]
  });
}
...
function removeEquipmentForTrade(userId,inventoryId){
 ...
 if(!row||(!row.slot&&row.item_type!=='artifact')||Number(row.quantity)<=0)return null;
...
function allEquipmentCatalog(limit=25,query=''){
 ...
 if(q)return db.prepare(`SELECT item_key key,name,rarity,slot,item_type FROM hero_items WHERE (slot IS NOT NULL OR item_type='artifact') AND LOWER(name) LIKE LOWER(?) ORDER BY name LIMIT ?`).all(`%${q}%`,Number(limit));
 return db.prepare(`SELECT item_key key,name,rarity,slot,item_type FROM hero_items WHERE (slot IS NOT NULL OR item_type='artifact') ORDER BY name LIMIT ?`).all(Number(limit));
}
...
async function showMarketBuy(interaction,notice=''){
  ...
  const components=[];
  if(eq.length)components.push(new ActionRowBuilder().addComponents(new StringSelectMenuBuilder().setCustomId('guild:orders:buy:eq').setPlaceholder('Выбери предмет или зелье для покупки').addOptions(eq.map(x=>({label:`${x.item_name}${Number(x.quantity)>1?` ×${x.quantity}`:''} — ${x.price} Dust`.slice(0,100),description:`Лот №${x.id} • ${x.rarity}${x.upgrade_level?` • +${x.upgrade_level}`:''}${x.item_type==='consumable'?' • зелье/расходник':''}`.slice(0,100),value:String(x.id),emoji:'⚔️'})))));
  ...
  return interaction.update({content:'',embeds:[new EmbedBuilder().setColor(0x22C55E).setTitle('🛒 Покупка').setDescription([notice,eq.length||companions.length?'Выбери экипировку, артефакт, питомца или маунта. Покупка будет показана для подтверждения.':'Сейчас нет доступных лотов.',orders.length?`\n📋 Активных заказов на материалы: **${orders.length}**. Они находятся в разделе «Заказы».`:'' ].filter(Boolean).join('\n'))],components});
}
async function showMarketSell(interaction,notice=''){
 const resources=listResources(interaction.user.id).filter(x=>x.quantity>0).slice(0,25), duplicates=duplicateEquipment(interaction.user.id).slice(0,25), consumables=marketConsumables(interaction.user.id).slice(0,25), companions=sellableCompanions(interaction.user.id).slice(0,25);const components=[];
 ...
 if(duplicates.length)components.push(new ActionRowBuilder().addComponents(new StringSelectMenuBuilder().setCustomId('guild:orders:sell:eq').setPlaceholder('Выставить предмет или артефакт').addOptions(duplicates.map(x=>({label:`${x.name} • свободно ${x.sellable}`.slice(0,100),value:String(x.id),emoji:x.item_type==='artifact'?'💍':'⚔️',description:`${x.item_type==='artifact'?'Артефакт • ':''}${x.rarity}${x.upgrade_level?` • +${x.upgrade_level}`:''}`.slice(0,100)})))));
 if(consumables.length)components.push(new ActionRowBuilder().addComponents(new StringSelectMenuBuilder().setCustomId('guild:orders:sell:consumable').setPlaceholder('Выставить зелья и расходники').addOptions(consumables.map(x=>({label:`${x.name} ×${x.quantity}`.slice(0,100),value:String(x.id),emoji:'🧪',description:`Игрок сам задаёт цену за всю выбранную стопку`.slice(0,100)})))));
 ...
 components.push(new ActionRowBuilder().addComponents(
  new ButtonBuilder().setCustomId('guild:orders:sell:blacksmith').setLabel('Продать кузнецу').setEmoji('🔨').setStyle(ButtonStyle.Secondary),
  new ButtonBuilder().setCustomId('guild:orders:requests:create').setLabel('Создать заказ').setEmoji('📋').setStyle(ButtonStyle.Primary),
  new ButtonBuilder().setCustomId('guild:orders').setLabel('Назад на рынок').setEmoji('↩️').setStyle(ButtonStyle.Secondary)
 ));
 const lines=resources.map(x=>`${x.icon} **${x.name}** ×${x.quantity}`).join('\n')||'Материалов нет.';
 return interaction.update({content:'',embeds:[new EmbedBuilder().setColor(0xF59E0B).setTitle('💰 Продажа').setDescription([notice,'### Материалы',lines,'','Выбери материал — бот покажет подходящие заказы и предложит количество.','Экипировку, артефакты, зелья, расходники, неактивных питомцев и маунтов можно выставить на рынок через меню ниже. Цену игрок задаёт сам.'].filter(Boolean).join('\n'))],components});
}
...
function merchantHomeRows() {
  return [
    new ActionRowBuilder().addComponents(
      ...
      new ButtonBuilder().setCustomId('guild:merchant:sell:consumables').setLabel('Продать зелья').setEmoji('🧪').setStyle(ButtonStyle.Success),
...
function showMerchantEquipmentList(interaction,notice=''){
  ...
  const menu=new StringSelectMenuBuilder().setCustomId('guild:merchant:sell:equipment:select').setPlaceholder('Выбери предмет для оценки').addOptions(rows.map(i=>({label:i.name.slice(0,100),value:String(i.id),description:`${RARITY_LABELS[i.rarity]||i.rarity}${i.upgrade_level?` · +${i.upgrade_level}`:''} · свободно ${i.sellableQuantity} · ${i.unitPrice} Dust`})));
...
function showMerchantEquipmentConfirm(interaction,id){
  ...
  return interaction.update({content:'',embeds:[new EmbedBuilder().setColor(0xDC2626).setTitle(`⚔️ ${item.name}`).setDescription(`Редкость: **${RARITY_LABELS[item.rarity]||item.rarity}**\nУлучшение: **+${item.upgrade_level||0}**\nСвободных экземпляров: **${item.sellableQuantity}**\nОценка торговца: **${item.unitPrice} GS Dust**\n\n⚠️ Продажа необратима.`)],components:[new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`guild:merchant:sell:equipment:confirm:${item.id}`).setLabel(`Продать за ${item.unitPrice}`).setEmoji('💠').setStyle(ButtonStyle.Danger),new ButtonBuilder().setCustomId('guild:merchant:sell:equipment').setLabel('Отмена').setStyle(ButtonStyle.Secondary))]});
}

function showMerchantConsumableList(interaction,notice=''){
  const rows=guildMerchant.sellableConsumables(interaction.user.id).slice(0,25);
  if(!rows.length)return interaction.update({content:`${notice}

🧪 У тебя нет зелий или других расходников для продажи.`,embeds:[],components:[new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('guild:merchant').setLabel('Назад').setEmoji('↩️').setStyle(ButtonStyle.Secondary))]});
  const menu=new StringSelectMenuBuilder().setCustomId('guild:merchant:sell:consumable:select').setPlaceholder('Выбери зелье или расходник').addOptions(rows.map(i=>({label:i.name.slice(0,100),value:String(i.id),emoji:'🧪',description:`${RARITY_LABELS[i.rarity]||i.rarity} · есть ${i.quantity} · ${i.unitPrice} Dust за 1`.slice(0,100)})));
  return interaction.update({content:'',embeds:[new EmbedBuilder().setColor(0x10B981).setTitle('🧪 Продажа зелий и расходников').setDescription(`${notice?`${notice}

`:''}Можно продать одно, несколько или все имеющиеся зелья. Цена зависит от редкости.`)],components:[new ActionRowBuilder().addComponents(menu),new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('guild:merchant').setLabel('Назад').setEmoji('↩️').setStyle(ButtonStyle.Secondary))]});
}
function showMerchantConsumableConfirm(interaction,id,notice=''){
  const item=guildMerchant.sellableConsumables(interaction.user.id).find(x=>Number(x.id)===Number(id));
  if(!item)return showMerchantConsumableList(interaction,'❌ Расходник уже закончился.');
  const max=Math.max(1,Number(item.quantity)||1);
  const buttons=[
    new ButtonBuilder().setCustomId(`guild:merchant:sell:consumable:${item.id}:1`).setLabel('Продать 1').setStyle(ButtonStyle.Success),
    new ButtonBuilder().setCustomId(`guild:merchant:sell:consumable:${item.id}:${Math.min(5,max)}`).setLabel(`Продать ${Math.min(5,max)}`).setStyle(ButtonStyle.Success).setDisabled(max<2),
    new ButtonBuilder().setCustomId(`guild:merchant:sell:consumable:${item.id}:${max}`).setLabel(`Продать всё (${max})`).setStyle(ButtonStyle.Danger),
    new ButtonBuilder().setCustomId('guild:merchant:sell:consumables').setLabel('Назад').setStyle(ButtonStyle.Secondary),
  ];
  return interaction.update({content:'',embeds:[new EmbedBuilder().setColor(0x059669).setTitle(`🧪 ${item.name}`).setDescription(`${notice?`${notice}

`:''}В наличии: **${max}**
Цена за единицу: **${item.unitPrice} GS Dust**
Цена за всё: **${item.unitPrice*max} GS Dust**

⚠️ Продажа необратима.`)],components:[new ActionRowBuilder().addComponents(...buttons)]});
}
...
function giftAssetList(interaction,type,notice='') {
  ...
  else rows=getInventory(interaction.user.id,{limit:200}).filter(x=>x.quantity>0).map(x=>({label:x.name,value:String(x.id),description:`${x.item_type==='artifact'?'Артефакт':'Предмет'} • доступно ${x.quantity}`,emoji:x.item_type==='artifact'?'💍':'🎒'}));
...
async function handleComponent(interaction) {
  ...
  if (action === 'merchant' && parts[2] === 'sell' && parts[3] === 'consumables') return showMerchantConsumableList(interaction);
  if (action === 'merchant' && parts[2] === 'sell' && parts[3] === 'consumable' && parts[4] === 'select') return showMerchantConsumableConfirm(interaction,interaction.values?.[0]);
  if (action === 'merchant' && parts[2] === 'sell' && parts[3] === 'consumable' && parts[4] && parts[4] !== 'select') {
    const id=Number(parts[4]),qty=Number(parts[5]||1);
    const r=guildMerchant.sellConsumable(interaction.user.id,id,qty);
    return showMerchantConsumableList(interaction,r.ok?`✅ Продано **${r.item.name} ×${r.qty}** за **${r.total} GS Dust**.`:'❌ Продажа не выполнена.');
  }
...
Подтвердить смену профессии?`,embeds:[],components:[new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`guild:profession:change:confirm:${nextKey}`).setLabel('Подтвердить').setEmoji('✅').setStyle(ButtonStyle.Danger),new ButtonBuilder().setCustomId('guild:profession').setLabel('Отмена').setEmoji('❌').setStyle(ButtonStyle.Secondary))]});
  ...
  if (action === 'orders' && parts[2] === 'buy' && parts[3] === 'eq') {const id=Number(interaction.values[0]);const l=listEquipmentMarket(100).find(x=>x.id===id);if(!l)return showMarketBuy(interaction,'❌ Лот уже недоступен.');return interaction.update({embeds:[new EmbedBuilder().setColor(0x22C55E).setTitle('🛒 Подтвердить покупку').setDescription(formatItemDescription({...l,name:l.item_name},{price:l.price,sellerId:l.seller_id}))],components:[new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`guild:orders:buy:confirm:${id}`).setLabel(`Купить за ${l.price}`).setEmoji('✅').setStyle(ButtonStyle.Success),new ButtonBuilder().setCustomId('guild:orders:buy').setLabel('Отмена').setStyle(ButtonStyle.Secondary))]});}
  ...
  if (action === 'orders' && parts[2] === 'sell' && parts[3] === 'consumable') {const item=getInventoryItem(interaction.user.id,Number(interaction.values[0]));if(!item)return showMarketSell(interaction,'❌ Предмет уже недоступен.');return interaction.showModal(marketModal(`guild:market:consumablesell:${item.id}`,'Выставить зелья',[{id:'quantity',label:`Количество (макс. ${item.quantity})`,placeholder:'Например: 3'},{id:'price',label:'Цена за всю стопку',placeholder:'Например: 120'}]));}
  ...
  if (action === 'orders' && parts[2] === 'requests' && parts[3] === 'create') {const opts=marketMaterialEntries().slice(0,25).map(([key,x])=>({label:(x.name||key).slice(0,100),value:key,emoji:x.icon||'📦'}));return interaction.update({embeds:[new EmbedBuilder().setColor(0x8B5CF6).setTitle('Создать заказ').setDescription('Выбери материал, затем укажи количество и цену за единицу.')],components:[new ActionRowBuilder().addComponents(new StringSelectMenuBuilder().setCustomId('guild:orders:requests:createitem').setPlaceholder('Выбери материал').addOptions(opts)),marketBackRow()]});}
  ...
  if (action === 'orders' && parts[2] === 'exchange' && parts[3] === 'take') {const lot=db.prepare("SELECT * FROM market_exchange_lots WHERE id=? AND status='open'").get(Number(interaction.values[0]));if(!lot)return showExchangeHub(interaction,'❌ Предложение уже закрыто.');return interaction.update({embeds:[new EmbedBuilder().setColor(0xEC4899).setTitle(lot.want_type==='free'?'Получить подарок':'Подтвердить обмен').setDescription(lot.want_type==='free'?`Ты ничего не отдаёшь.\nПолучаешь: **${lot.offer_name} ×${lot.offer_qty}**`:`Ты отдаёшь: **${lot.want_name} ×${lot.want_qty}**\nПолучаешь: **${lot.offer_name} ×${lot.offer_qty}**`)],components:[new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`guild:orders:exchange:confirm:${lot.id}`).setLabel(lot.want_type==='free'?'Получить':'Обменять').setEmoji(lot.want_type==='free'?'🎁':'✅').setStyle(ButtonStyle.Success),new ButtonBuilder().setCustomId('guild:orders:exchange').setLabel('Отмена').setStyle(ButtonStyle.Secondary))]});}
  if (action === 'orders' && parts[2] === 'exchange' && parts[3] === 'confirm') {
    ...
    try{
      completed=db.transaction(()=>{
        ...
        if(lot.want_type==='free'){
          // Бесплатная передача: принимающий игрок ничего не отдаёт.
        }else if(lot.want_type==='material'){
        ...
        }else if(lot.want_type==='consumable'){const row=db.prepare('SELECT * FROM hero_inventory WHERE user_id=? AND item_key=? AND quantity>=?').get(takerId,lot.want_key,lot.want_qty);if(!row)throw new Error('consumable');const ch=Number(row.quantity)===Number(lot.want_qty)?db.prepare('DELETE FROM hero_inventory WHERE id=? AND user_id=?').run(row.id,takerId):db.prepare('UPDATE hero_inventory SET quantity=quantity-? WHERE id=? AND user_id=? AND quantity>=?').run(lot.want_qty,row.id,takerId,lot.want_qty);if(!ch.changes)throw new Error('consumable');grantItem(lot.creator_id,lot.want_key,lot.want_qty,'market_exchange_wanted');}else if(lot.want_type==='companion'){
        ...
        else if(lot.offer_type==='consumable')grantItem(takerId,lot.offer_key,Number(lot.offer_qty)||1,'market_exchange_offer');
    ...
    const errors={dust:'❌ Недостаточно GS Dust.',materials:'❌ Недостаточно материалов.',companion:'❌ Нужный питомец или маунт не найден либо активен.',equipment:'❌ Нужный предмет не найден.',consumable:'❌ Нужного количества зелий или расходников нет.',closed:'❌ Предложение уже закрыто.',self:'❌ Нельзя принять собственный обмен.'};
  ...
  if (action === 'inventory' && parts[2] === 'treasuremap') {
    const result=activateTreasureMap(interaction.user.id);
    const notice=result.ok
      ? '✅ Карта сокровищ активирована. Она будет автоматически использована при старте следующей экспедиции и даст дополнительный тайник.'
      : result.reason==='already_active'
        ? 'ℹ️ Карта сокровищ уже активирована для следующей экспедиции.'
        : '❌ В инвентаре нет Карты сокровищ.';
    return showInventory(interaction,notice);
  }
  ...
  if (action === 'blacksmith' && parts[2] === 'dismantleconfirm') { const r=dismantle(interaction.user.id,Number(parts[3])); return showBlacksmith(interaction,r.ok?`✅ **${r.item.name}** разобран. Получено: **${r.name} ×${r.qty}**.`:'❌ Не удалось разобрать предмет. Единственный надетый экземпляр нужно сначала снять; если копий две или больше, перерабатывается свободная копия.'); }
  ...
  if (action === 'artifacts' && parts.length===2) return showArtifacts(interaction);
  if (action === 'artifacts' && parts[2] === 'equipselect') return showArtifactSlotChoice(interaction,interaction.values?.[0]);
  if (action === 'artifacts' && parts[2] === 'equip') {
    const result=equipArtifact(interaction.user.id,Number(parts[3]),Number(parts[4]));
    return showArtifacts(interaction,result.ok?`✅ **${result.row.name}** экипирован в слот ${result.slot}.`:'❌ Не удалось экипировать артефакт.');
  }
  if (action === 'artifacts' && parts[2] === 'remove') {
    const result=unequipArtifact(interaction.user.id,Number(interaction.values?.[0]));
    return showArtifacts(interaction,result.ok?'✅ Артефакт снят.':'❌ Не удалось снять артефакт.');
  }
...
async function handleModal(interaction) {
  ...
  if(parts[0]==='guild'&&parts[1]==='market'){
    ...
    if(kind==='consumablesell'){const quantity=Number(interaction.fields.getTextInputValue('quantity'));const price=Number(interaction.fields.getTextInputValue('price'));const r=createListing(interaction.user.id,Number(parts[3]),price,quantity);return interaction.reply({content:r.ok?`✅ **${r.listing.item_name} ×${r.listing.quantity}** выставлено за **${r.listing.price} GS Dust**.`:'❌ Не удалось выставить зелья. Проверь количество, цену и наличие.',flags:MessageFlags.Ephemeral});}
    ...
    if(kind==='exchangesearch'){
      ...
      const menu=new StringSelectMenuBuilder().setCustomId('guild:orders:exchange:wanted:equipment').setPlaceholder('Выбери найденный предмет').addOptions(matches.map(([key,x])=>({label:(x.name||key).slice(0,100),value:key,emoji:x.item_type==='artifact'?'💍':'⚔️',description:(x.item_type==='artifact'?'Артефакт • ':'')+(x.rarity||'equipment').slice(0,100)})));
    ...
    if(kind==='exchangeqty'){const d=marketDrafts.get(interaction.user.id);if(!d)return interaction.reply({content:'❌ Черновик обмена устарел.',flags:MessageFlags.Ephemeral});let oq=Math.max(1,Number(interaction.fields.getTextInputValue('offer_qty'))||1),wq=d.wantType==='free'?0:Math.max(1,Number(interaction.fields.getTextInputValue('want_qty'))||1);if(['equipment','companion'].includes(d.offerType))oq=1;if(['equipment','companion'].includes(d.wantType))wq=1;let offerKey,offerName,upgrade=0,offerData=null;try{db.transaction(()=>{if(d.offerType==='material'){offerKey=d.offerValue;offerName=itemName(offerKey);if(!consumeResources(interaction.user.id,{[offerKey]:oq}).ok)throw new Error('materials');}else if(d.offerType==='dust'){offerKey='gs_dust';offerName='GS Dust';const payment=removeCardDust(interaction.user.id,oq);if(!payment?.ok)throw new Error('dust');}else if(d.offerType==='consumable'){const item=getInventoryItem(interaction.user.id,Number(d.offerValue));if(!item||Number(item.quantity)<oq||(item.item_type!=='consumable'&&item.type!=='consumable'))throw new Error('consumable');offerKey=item.item_key;offerName=item.name;const changed=Number(item.quantity)===oq?db.prepare('DELETE FROM hero_inventory WHERE id=? AND user_id=?').run(item.id,interaction.user.id):db.prepare('UPDATE hero_inventory SET quantity=quantity-? WHERE id=? AND user_id=? AND quantity>=?').run(oq,item.id,interaction.user.id,oq);if(!changed.changes)throw new Error('consumable');}else if(d.offerType==='companion'){const removed=takeCompanionForTransfer(interaction.user.id,Number(d.offerValue));if(!removed)throw new Error('companion');offerKey=removed.key;offerName=removed.name;offerData=JSON.stringify(removed);}else{const removed=removeEquipmentForTrade(interaction.user.id,Number(d.offerValue));if(!removed)throw new Error('equipment');offerKey=removed.key;offerName=removed.name;upgrade=removed.upgrade;}db.prepare(`INSERT INTO market_exchange_lots(creator_id,offer_type,offer_key,offer_name,offer_qty,offer_upgrade,want_type,want_key,want_name,want_qty,offer_data) VALUES(?,?,?,?,?,?,?,?,?,?,?)`).run(interaction.user.id,d.offerType,offerKey,offerName,oq,upgrade,d.wantType,d.wantKey,d.wantType==='free'?'Ничего':itemName(d.wantKey),wq,offerData);})();}catch(e){return interaction.reply({content:'❌ Не удалось зарезервировать предметы. Проверь количество и наличие предмета.',flags:MessageFlags.Ephemeral});}marketDrafts.delete(interaction.user.id);return interaction.reply({content:d.wantType==='free'?`✅ Бесплатная передача создана: **${offerName} ×${oq}**. Забрать сможет другой игрок через раздел обменов.`:`✅ Предложение обмена создано: **${offerName} ×${oq} ⇄ ${itemName(d.wantKey)} ×${wq}**.`,flags:MessageFlags.Ephemeral});}
  }
