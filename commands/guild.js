...
function removeEquipmentForTrade(userId,inventoryId){const row=db.prepare('SELECT * FROM hero_inventory WHERE id=? AND user_id=?').get(inventoryId,String(userId));if(!row||!ITEMS[row.item_key]?.slot||Number(row.quantity)<=0)return null;const equipped=db.prepare('SELECT 1 FROM hero_equipment WHERE user_id=? AND inventory_id=? UNION SELECT 1 FROM hero_class_equipment WHERE user_id=? AND inventory_id=? LIMIT 1').get(String(userId),inventoryId,String(userId),inventoryId);const reserved=equipped?1:0;if(Number(row.quantity)-reserved<1)return null;if(Number(row.quantity)<=1)db.prepare('DELETE FROM hero_inventory WHERE id=?').run(inventoryId);else db.prepare('UPDATE hero_inventory SET quantity=quantity-1 WHERE id=?').run(inventoryId);return {key:row.item_key,name:ITEMS[row.item_key]?.name||row.item_key,upgrade:Number(row.upgrade_level||0)};}
