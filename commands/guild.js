...
const { listCompanions, activateCompanion, sellableCompanions, takeCompanionForTransfer, giveTransferredCompanion } = require('../systems/hero/companionService');
...
async function showInventory(interaction, notice = '') {
  ...
  const slotText=slots.map(slot=>{const x=bySlot.get(slot);return `${icons[slot]||'▫️'} **${labels[slot]}:** ${x?`🟢 ${x.name}${Number(x.upgrade_level||0)?` +${x.upgrade_level}`:''}`:'⚪ Свободно'}`;}).join('\n');
  const legacy=equipment.filter(x=>!slots.includes(x.slot)).map(x=>`${icons[x.slot]||'🟢'} **${labels[x.slot]||x.slot}:** 🟢 ${x.name}`).join('\n');
  const itemText=items.length?items.slice(0,25).map(i=>`${equippedIds.has(Number(i.id))?'🟢':i.slot?'⚪':'📦'} **#${i.id} ${i.name}**${Number(i.upgrade_level||0)?` +${i.upgrade_level}`:''} ×${i.quantity} · ${RARITY_LABELS[i.rarity]||i.rarity}`).join('\n'):'Инвентарь пока пуст.';
  const embed=new EmbedBuilder().setColor(0x8B5CF6).setTitle(`🎒 Инвентарь — ${hero.name}`).setDescription([notice,'### Активные слоты',slotText,legacy,'','### Предметы',itemText].filter(Boolean).join('\n').slice(0,4000)).setFooter({text:'🟢 активно • ⚪ свободно. Можно надеть 2 кольца, ближнее и дальнее оружие одновременно, а также щит.'});
  const components=[],eq=items.filter(i=>i.slot).slice(0,25);if(eq.length)components.push(new ActionRowBuilder().addComponents(new StringSelectMenuBuilder().setCustomId('guild:inventory:select').setPlaceholder('Выбрать предмет').addOptions(eq.map(i=>({label:`#${i.id} ${i.name}`.slice(0,100),value:String(i.id),emoji:equippedIds.has(Number(i.id))?'🟢':'⚔️',description:equippedIds.has(Number(i.id))?'Сейчас активно':'Свободно в инвентаре'})))));components.push(new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('guild:home').setLabel('Вернуться в Гильдию').setEmoji('⬅️').setStyle(ButtonStyle.Secondary)));
