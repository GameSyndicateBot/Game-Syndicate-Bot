...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const active = buffs.length ? buffs.map(b => { const effect = ALCHEMY_EFFECTS[b.buff_key] || ALCHEMY_EFFECTS[b.source_item_key]; return `✨ **${effect ? `${effect.icon} ${effect.name}` : b.source_item_key}** · ${b.context === 'world_boss' ? 'World Boss' : 'экспедиция'} · зарядов: ${b.charges}\n${bonusText(b.bonuses)}`; }).join('\n\n') : 'Активных эффектов нет.';
